import { Injectable } from '@angular/core';
import { HttpResponse } from '@angular/common/http';
import { Resolve, ActivatedRouteSnapshot, Routes, Router } from '@angular/router';
import { Observable, of, EMPTY } from 'rxjs';
import { flatMap } from 'rxjs/operators';

import { Authority } from 'app/shared/constants/authority.constants';
import { UserRouteAccessService } from 'app/core/auth/user-route-access-service';
import { ITypeAbsence, TypeAbsence } from 'app/shared/model/type-absence.model';
import { TypeAbsenceService } from './type-absence.service';
import { TypeAbsenceComponent } from './type-absence.component';
import { TypeAbsenceDetailComponent } from './type-absence-detail.component';
import { TypeAbsenceUpdateComponent } from './type-absence-update.component';

@Injectable({ providedIn: 'root' })
export class TypeAbsenceResolve implements Resolve<ITypeAbsence> {
  constructor(private service: TypeAbsenceService, private router: Router) {}

  resolve(route: ActivatedRouteSnapshot): Observable<ITypeAbsence> | Observable<never> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        flatMap((typeAbsence: HttpResponse<TypeAbsence>) => {
          if (typeAbsence.body) {
            return of(typeAbsence.body);
          } else {
            this.router.navigate(['404']);
            return EMPTY;
          }
        })
      );
    }
    return of(new TypeAbsence());
  }
}

export const typeAbsenceRoute: Routes = [
  {
    path: '',
    component: TypeAbsenceComponent,
    data: {
      authorities: [Authority.USER],
      defaultSort: 'id,asc',
      pageTitle: 'sigrhpApp.typeAbsence.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/view',
    component: TypeAbsenceDetailComponent,
    resolve: {
      typeAbsence: TypeAbsenceResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'sigrhpApp.typeAbsence.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: 'new',
    component: TypeAbsenceUpdateComponent,
    resolve: {
      typeAbsence: TypeAbsenceResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'sigrhpApp.typeAbsence.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
  {
    path: ':id/edit',
    component: TypeAbsenceUpdateComponent,
    resolve: {
      typeAbsence: TypeAbsenceResolve,
    },
    data: {
      authorities: [Authority.USER],
      pageTitle: 'sigrhpApp.typeAbsence.home.title',
    },
    canActivate: [UserRouteAccessService],
  },
];
