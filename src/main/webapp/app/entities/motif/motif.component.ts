...
export class MotifComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.motifService
      ...
      .subscribe(
        (res: HttpResponse<IMotif[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IMotif): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInMotifs(): void {
    this.eventSubscriber = this.eventManager.subscribe('motifListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IMotif[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
