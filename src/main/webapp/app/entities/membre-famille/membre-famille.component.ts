...
export class MembreFamilleComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.membreFamilleService
      ...
      .subscribe(
        (res: HttpResponse<IMembreFamille[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IMembreFamille): number {
  ...
  registerChangeInMembreFamilles(): void {
    this.eventSubscriber = this.eventManager.subscribe('membreFamilleListModification', () => this.loadPage());
  }

  delete(membreFamille: IMembreFamille): void {
    const modalRef = this.modalService.open(MembreFamilleDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IMembreFamille[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
