...
export class BulletinComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.bulletinService
      ...
      .subscribe(
        (res: HttpResponse<IBulletin[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IBulletin): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInBulletins(): void {
    this.eventSubscriber = this.eventManager.subscribe('bulletinListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IBulletin[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
