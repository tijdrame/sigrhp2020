...
export class RemboursementComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.remboursementService
      ...
      .subscribe(
        (res: HttpResponse<IRemboursement[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  trackId(index: number, item: IRemboursement): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInRemboursements(): void {
    this.eventSubscriber = this.eventManager.subscribe('remboursementListModification', () => this.loadPage());
  }

  delete(remboursement: IRemboursement): void {
    const modalRef = this.modalService.open(RemboursementDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IRemboursement[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
