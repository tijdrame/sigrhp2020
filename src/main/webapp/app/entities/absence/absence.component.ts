...
export class AbsenceComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.absenceService
      ...
      .subscribe(
        (res: HttpResponse<IAbsence[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
        () => this.onError()
      );
  }
  ...
  registerChangeInAbsences(): void {
    this.eventSubscriber = this.eventManager.subscribe('absenceListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IAbsence[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
